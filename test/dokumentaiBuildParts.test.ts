...
// Paskutinis atnaujinimas: pavadinimas ir autorius indeksuojami ir sulietuvinti
// (`titleAscii`/`authorAscii`), tad boost'as taikomas jiems, o koordinatės
// persikėlė į `geo` JSON lauką kartu su Morton raktais.
const RICH = {
  q: 'sutartis',
  klase: 'a,b',
...
describe('buildParts characterization (query-string stability)', () => {
  ...
  it('pavadinimą sveria labiau už turinį, bazinės sąlygos nesiaurindamas', () => {
    ...
    expect(buildPartsExcluding(parsed)).toBe(
      '(("silumos" "tinklu") OR titleAscii:("silumos" "tinklu")^6)',
    );
  });
  ...
  it('be diakritikos nededa perteklinės žalios formos', () => {
    ...
    expect(query).toBe('(("sutartis") OR titleAscii:("sutartis")^6)');
    expect(query).not.toContain('author:');
  });

  it('su diakritika prideda nesulietuvintą pavadinimo formą', () => {
    // `titleAscii` ir `authorAscii` indeksuojami sulietuvinti, kaip ir `text`,
    // tad bazinė sąlyga pasiekia visus tris. Žalia forma lieka tik `title`,
    // kad rastųsi tiksli atitiktis su diakritika; autoriui jos nebereikia.
    const parsed = buildPartsOpts({ q: 'šilumos tinklų' });
    const query = buildPartsExcluding(parsed);
    expect(query).toBe(
      '(("silumos" "tinklu")' +
      ' OR titleAscii:("silumos" "tinklu")^6' +
  ...
  it('frazės režimu boost\'ina visą frazę', () => {
    ...
    expect(buildPartsExcluding(parsed)).toBe(
      ...
      ' OR titleAscii:("silumos tinklu rekonstrukcija")^6' +
  ...
  it('tekstas lieka paskutinis AND narys po filtrų', () => {
    ...
    expect(buildPartsExcluding(parsed)).toBe(
      '(class:"teisekura") AND (("silumos") OR titleAscii:("silumos")^6' +
      ' OR title:("šilumos")^6)',
    );
  });
});
