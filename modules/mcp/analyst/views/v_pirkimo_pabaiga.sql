...
--
-- Deliberately does NOT go through v_dalyviai: that view only produces a row
-- when xlsxPPAdalyviai has a matching participant, so a procedure that ended
-- because no supplier submitted anything at all ("Per nustatytą terminą
-- tiekėjams nepateikus nė vienos paraiškos, pasiūlymo...") would silently
-- disappear — exactly the outcome this entity exists to keep. Reading
-- xlsxPPAataskaitos/xlsxPPAproceduruPabaiga directly avoids that.
--
...
CREATE OR REPLACE VIEW v_pirkimo_pabaiga AS
