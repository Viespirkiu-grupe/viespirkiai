...
const scrapeFetch = createScraperFetch("eSeimas", { operation: "eSeimasApi" });
import config from "../../utils/config.js";
import { log } from "../../utils/log.js";
import { sleep } from "../../utils/time.js";

// Klientas „Stateless e-Seimas API" adapteriui — jo OpenAPI: <ETAR_API_URL>/openapi.json.
// Adapteris pats eina į e-Seimas gyvai ir nieko nesaugo, tad visas kešavimas ir
...
export class ESeimasApiError extends Error {
    constructor(message, { status, url, body, retryAfterMs } = {}) {
        super(message);
        this.name = "ESeimasApiError";
...
/**
 * e-Seimas aiškiai sako, kad prašomo dokumento nėra. Du atvejai, skiriami pagal
 * `body.error`:
 ...
 *      e-Seimas tuo adresu jau grąžina bendrą redakcijų sąrašą; šviežio tokeno
 *      reikia ieškoti /editions atsakyme (žr. eSeimasScrape.scrapeHistoricalEdition).
 */
export class ESeimasNotFoundError extends ESeimasApiError {
    constructor(message, details) {
        super(message, details);
        this.name = "ESeimasNotFoundError";
    }
}
...
export function createESeimasApi({
    ...
    if (!baseUrl) {
        throw new Error("Nenustatytas ETAR_API_URL (config.eTarApiUrl) — e-Seimas adapterio adresas");
    }
    ...
    function markOk() {
        if (busySince) {
            log(`e-Seimas adapteris atsigavo po ${Math.round((Date.now() - busySince) / 1000)} s`
    ...
    async function request(pathname, params) {
        ...
        for (let busyAttempt = 0;;) {
            ...
            if (!(error instanceof ESeimasApiError) || !BUSY_STATUSES.has(error.status)) throw error;
            ...
            if (Date.now() - busyLoggedAt > BUSY_LOG_INTERVAL) {
                ...
                log(`e-Seimas adapteris užimtas (${error.status}) — stabdoma`
    ...
    async function requestOnce(url) {
        ...
        if (!res.ok) {
            const message = `e-Seimas API ${res.status} ${url.pathname}${body?.error ? `: ${body.error}` : ""}`
                + (body?.message ? ` — ${body.message}` : "");
            const details = { status: res.status, url: url.href, body, retryAfterMs: retryAfterMs(res) };
            throw res.status === 404 ? new ESeimasNotFoundError(message, details) : new ESeimasApiError(message, details);
        }
        if (body == null) {
            throw new ESeimasApiError(`e-Seimas API grąžino ne JSON (${url.pathname})`, { status: res.status, url: url.href });
        }
        return body;
    }
    ...
    return {
        ...
        /** Vienas 50 eilučių e-Seimo puslapis; `page` yra 1-based. */
        searchLegalActs({ from = "", to = "", page = 1 } = {}) {
            return request("/v1/seimas/legal-acts", { from, to, page });
        },

        /** Originalus aktas. */
        getLegalAct(category, id) {
            return request(`/v1/seimas/legal-acts/${encodeURIComponent(category)}/${encodeURIComponent(id)}`);
        },

        /** Galiojanti suvestinė redakcija; meta ESeimasNotFoundError, jei jos nėra. */
        getConsolidatedEdition(category, id) {
            return request(`/v1/seimas/legal-acts/${encodeURIComponent(category)}/${encodeURIComponent(id)}/asr`);
        },

        /** Suvestinių redakcijų sąrašas pagal datą. */
        getEditionList(category, id) {
            return request(`/v1/seimas/legal-acts/${encodeURIComponent(category)}/${encodeURIComponent(id)}/editions`);
        },

        /** Konkreti istorinė suvestinė; `edition` — neskaidrus tokenas iš /editions. */
        getHistoricalConsolidatedEdition(category, id, edition) {
            return request(`/v1/seimas/legal-acts/${encodeURIComponent(category)}/${encodeURIComponent(id)}/${encodeURIComponent(edition)}`);
        },
    };
}
