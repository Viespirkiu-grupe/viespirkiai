...
// e-Seimas API atsakymų sidecar saugykla.
//
// Postgres'e (eSeimas* lentelėse) laikom tik tai, ką galima normalizuoti. Pilnas
...
//               nerodydavo visai.
//
// Abu keliai kalba su ta pačia lentele: registre `eSeimas` turi `keyColumn: "md5"`,
// o schemos sutampa.
...
/** `<SIDECAR_DIR>/eSeimas.sqlite`; `null`, jei `SIDECAR_DIR` nenustatytas. */
export function getESeimasSidecarPath() {
    return sidecarDbPath("eSeimas");
}

/**
 * Atidaro (jei reikia – sukuria) e-Seimas sidecar bazę.
 * @param {Object} [opts]
 ...
 */
export function openESeimasSidecar({ dbPath = getESeimasSidecarPath(), readonly = false } = {}) {
    if (!dbPath) throw new Error("SIDECAR_DIR nenustatytas, negalima atidaryti eSeimas sidecar'o");
    return openSqlite({ dbPath, readonly, ensureSchema: ensureESeimasSidecarSchema });
}
...
export function writeRows(db, rows) {
    ...
    const stmt = db.prepare(
        `INSERT INTO "eSeimasAtsakymai" ("md5", "dydis", "suspaustas", "turinys")
         VALUES (?, ?, ?, ?)
 ...
 * (`eSeimasScrape`, `eSeimasAnomalijos`), kur bazė vis tiek jau atidaryta.
 * Skaitytojai naudoja `readESeimasSidecar`.
 * @returns {Object|null} atsakymas be nepastovių laukų
 */
export function readResponse(db, md5) {
    if (!md5) return null;
    const row = db.prepare(`SELECT "turinys" FROM "eSeimasAtsakymai" WHERE "md5" = ?`).get(md5);
    if (!row) return null;
...
const store = createSidecarStore({ sidecar: "eSeimas", label: "e-Seimas atsakymo" });

/** @param {string} md5 @returns {Promise<Object|null>} */
export const readESeimasSidecar = store.read;

/** Partija: `Map<md5, atsakymas>` tik su rastais. @param {string[]} md5s */
export const readESeimasSidecarMany = store.readMany;

export const isESeimasSidecarConfigured = store.localConfigured;

export function hasResponse(db, md5) {
    if (!md5) return false;
    return db.prepare(`SELECT 1 FROM "eSeimasAtsakymai" WHERE "md5" = ?`).get(md5) != null;
}

export function getESeimasSidecarStats(db) {
    const row = db
        .prepare(`SELECT COUNT(*) c, SUM("dydis") raw, SUM("suspaustas") zstd FROM "eSeimasAtsakymai"`)
        .get();
    return {
        count: Number(row.c),
