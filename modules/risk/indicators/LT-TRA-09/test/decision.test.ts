...
import { LtTra09Decision } from "../decision.ts";
...
// Unit tests for the judgement half of LT-TRA-09: plain objects in, plain
...
function procurementSubject(overrides: Partial<Procurement> = {}): ProcurementSubject {
    ...
    return {
        ...
        subjectKey: "cvpis:900901",
        procurementSource: "cvpis",
        procurementId: "900901",
        procurement,
    };
}
...
describe("LtTra09Decision.assessRisk", () => {
    ...
    it("is pure: the same scenario returns a deeply equal signal every time", () => {
        expect(assessRiskFor(false)).toEqual(assessRiskFor(false));
    });
});

describe("LtTra09Decision end to end (through RiskDecisionEngine, no database)", () => {
    const engine = new RiskDecisionEngine([ltTra09v1], CONTEXT);

    it("assembles a complete triggered signal from a procurement whose report says it was not electronic", () => {
    ...
    it("reports not_triggered for a report that positively says the procedure was electronic", () => {
    ...
    it("reports insufficient_data when no ATN-1 report was ever filed (procedureOutcome null)", () => {
        ...
        expect(signal.missingData).toEqual(["elektroninisPirkimas"]);
    });

    it("reports insufficient_data when a report was filed but never populated elektroninisPirkimas", () => {
