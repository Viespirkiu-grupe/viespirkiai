...
import { LtTra07Decision } from "../decision.ts";
...
// Unit tests for the judgement half of LT-TRA-07: plain objects in, plain
...
function procurementSubject(overrides: Partial<Procurement> = {}): ProcurementSubject {
    ...
    return {
        ...
        subjectKey: "cvpis:900701",
        procurementSource: "cvpis",
        procurementId: "900701",
        procurement,
    };
}
...
describe("LtTra07Decision end to end (through RiskDecisionEngine, no database)", () => {
    const engine = new RiskDecisionEngine([ltTra07v1], CONTEXT);

    it("assembles a complete triggered signal from a procurement whose report says a complaint was filed", () => {
    ...
    it("reports not_triggered for a report that positively says no complaint was filed", () => {
    ...
    it("reports insufficient_data when no ATN-1 report was ever filed (procedureOutcome null)", () => {
        ...
        expect(signal.missingData).toEqual(["pretenzijaPateikta"]);
    });

    it("reports insufficient_data when a report was filed but never populated pretenzijaPateikta", () => {
