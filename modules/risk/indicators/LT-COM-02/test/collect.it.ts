...
describe("LT-COM-02 collect.sql", () => {
    ...
    it("scopes a run to the subjects passed as $2", async () => {
        ...
        expect(rows.map((row) => row.subjectKey)).toEqual(["cvpis:900101:0"]);
    });
    ...
    it("hides a report recorded after the cutoff, and shows it at a later one", async () => {
        ...
        expect(later.map((row) => row.subjectKey)).toEqual(["cvpis:900107:0"]);
    });
...
describe("LT-COM-02 end to end", () => {
    ...
    function evaluate(dataAsOf = DATA_AS_OF): Promise<readonly RiskObservationV1[]> {
        return ltCom02v1.evaluate({ runId: 1, dataAsOf, subjects: null }, facts);
    }
