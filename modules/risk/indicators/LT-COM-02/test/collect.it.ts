// Integration test for LT-COM-02's collect.sql. Runs the real statement
...
import { ltCom02v1, type LtCom02Facts } from "../decision.ts";
import {
    duplicateBidderRows,
    fiveBidders,
    lateReport,
...
function sortKey(row: LtCom02Facts): string {
    return `${row.pirkimoNumeris}:${row.daliesNumeris}`;
}

/** collect.sql on its own, with the two arguments the shared class binds. */
async function collect(subjects: readonly string[] | null = null): Promise<readonly LtCom02Facts[]> {
    const rows = await facts.query<LtCom02Facts>(COLLECT_SQL, [DATA_AS_OF, subjects]);
    return [...rows].sort((a, b) => sortKey(a).localeCompare(sortKey(b)));
}

async function collectFor(fixture: ProcurementFixture): Promise<readonly LtCom02Facts[]> {
...
describe("LT-COM-02 collect.sql", () => {
    ...
    it.each([
        ["two participants", twoBidders],
        ["three participants", threeBidders],
        ["five participants", fiveBidders],
        ["an unmatched procurement", unmatchedProcurement],
        ["a multi-lot procurement", twoLotsDifferentBidderCounts],
    ...
    it("returns exactly one row per lot", async () => {
        await insertProcurement(twoLotsDifferentBidderCounts);
    ...
    it("scopes a run to the subjects passed as $2", async () => {
        await insertProcurement(twoBidders);
        await insertProcurement(fiveBidders);
        const rows = await collect(["900101"]);
        expect(rows.map((row) => row.pirkimoNumeris)).toEqual(["900101"]);
    });
    ...
    it("hides a report recorded after the cutoff, and shows it at a later one", async () => {
        ...
        const later = await facts.query<LtCom02Facts>(COLLECT_SQL, ["2026-10-01T00:00:00.000Z", null]);
        expect(later.map((row) => row.pirkimoNumeris)).toEqual(["900107"]);
    });
    ...
    it("decides nothing: no state, no identity, no threshold in the statement", () => {
        const withoutComments = COLLECT_SQL.replace(/--.*$/gm, "");
        expect(withoutComments).not.toMatch(/triggered|not_applicable|insufficient_data|LT-COM-02/);
    });
});

describe("LT-COM-02 end to end", () => {
    ...
    async function evaluateInsertedFixture(dataAsOf = DATA_AS_OF): Promise<readonly RiskSignal[]> {
        ...
        return ltCom02v1.evaluate({ runId: 1, dataAsOf, subjects: null }, lotSubjects, facts);
    }
    ...
    it("assembles a complete observation from a fact row and a decision", async () => {
        ...
        expect(observation).toEqual({
            ...
            threshold: { minimumBidders: 3 },
            appliedParameters: { minimumBidders: 3 },
            evidence: {
                pirkimoBudas: "Atviras konkursas",
                ataskaitosData: twoBidders.facts[0].reportedAt,
    ...
    // eligibility gate — not LtCom02Decision.decide — reports insufficient_data.
    it("reports insufficient_data via the shared eligibility gate for an unregistered procurement", async () => {
        ...
        expect(observation.subjectKey).toBe("unknown:900104:0");
    ...
    it("reports not_applicable with no applied parameters before the timeline starts", async () => {
        ...
        expect(observation.subjectKey).toBe("cvpis:900108:0");
