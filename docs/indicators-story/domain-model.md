...
| `v_pirkimas` → `v_sutartys`                           | **372,387 / 5,904,634 contract rows (6.3%)**; **88.5% of contracts have a NULL `pirkimoNumeris` entirely** (5,223,286 rows) | Only 307,239 distinct `pirkimoNumeris` values appear across all contracts. The NULLs are overwhelmingly `MVPŽ` (78.2% of all NULLs) and `MVP` (16.5%) — two legally-exempt, verbal/low-value types where CVP IS use isn't required or is optional — see [Problem 3](#high-problem-3-missing-v_sutartyspirkimonumeris) below. `TSP` and `PPS`, the two types legally required to have a number, account for ~186,000 of the 5.2M NULLs (3.6%); 68.4% / 55.8% of their rows are populated. That ~186,000-row slice is the real data-quality gap.                                                                                                    |
