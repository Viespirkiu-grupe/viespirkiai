...
The MCP analyst exposes seven views (`modules/mcp/analyst/views/*.sql`) as the domain model. This document treats each
view as a domain entity — no underlying tables are described here, only the views themselves, their key attributes, and
