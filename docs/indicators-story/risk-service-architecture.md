...
| # | Invariant                                                                                                                                                                                                                                                                                                                                     |
|---|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1 | **One row per procurement, refreshed in place.** There is no per-run snapshot; `risk."procurementDecisions"` is the current state. The writer needs `INSERT` **and** `UPDATE` on it, and `INSERT` **and** `DELETE` on `risk."signals"` (no `UPDATE` there — see Invariant 2).                                                                     |
...
| 9  | Grants: `risk_rw` insert-only on signals, DELETE for retention                                               | `migrations/risk/002_roles.sql`                                      | Superseded twice: first by `SELECT, INSERT, UPDATE` on the decisions/runs tables and `SELECT, INSERT, DELETE` on signals, then (2026-09) removed altogether — the `risk` schema is owned and used by the application's own role, and `001_risk.sql` creates no role and grants nothing |
