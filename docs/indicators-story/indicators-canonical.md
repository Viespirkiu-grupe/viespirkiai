...
| Lifecycle entity or transition | Canonical view | Backing table(s)                                                                                             | Identity or join mapping                                                                                                                         | Coverage / cardinality                                                                                                                                   |
|--------------------------------|----------------|--------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------|
| Buyer company                  | `v_pirkimas`   | `viesiejiPirkimai`, `viesiejiPirkimaiVykdytojai`; CVPP fallback via `cvppViesiejiPirkimai` and `vpmSutartys` | `v_pirkimas.jarKodas` identifies the buyer; CVPP fallback derives it from `vpmSutartys.perkanciosiosOrganizacijosKodas`                          | One buyer per procurement when the company code is available                                                                                             |
| Procurement                    | `v_pirkimas`   | `viesiejiPirkimai`; fallback `cvppViesiejiPirkimai`                                                          | Canonical key is `(saltinis, pirkimoId)`; `saltinis` is `cvpis` or `cvpp`                                                                        | One row per source procurement; CVP IS takes precedence over a matching CVPP notice                                                                      |
| Procurement lot                | `v_dalyviai`   | `atn1ataskaitos`, `atn1pasiulymuEile`, `atn1atmestiPasiulymai`                                               | Lot key is `(pirkimoNumeris, daliesNumeris)`; missing `daliesNumeris` is handled as the procedure-level/default lot by indicator implementations | Available only where an ATN-1 completion report and its structured detail were ingested                                                                  |
| Participant company            | `v_dalyviai`   | `atn1dalyviai`, optionally `jarAsmenys` for the company name                                                 | `tiekejoKodas` identifies the participant; `v_dalyviai.pirkimoNumeris = v_pirkimas.pirkimoId` links participation to procurement                 | One participant can appear in multiple lots; foreign or physical-person participants may not resolve to `jarAsmenys`                                     |
| Bid / proposal in a lot        | `v_dalyviai`   | `atn1pasiulymuEile`, `atn1atmestiPasiulymai`                                                                 | Natural key is `(pirkimoNumeris, daliesNumeris, tiekejoKodas)`; fields include `eileNumeris`, `pasiulymoKaina`, and `atmetimoPriezastis`         | Proposal detail is best-effort ATN-1 data, not complete for every procurement                                                                            |
| Award → winning supplier       | `v_sutartys`   | `vpmSutartys`, `vpmSutartysPapildomiTiekejai`, `vpmSutartysSalys`, optionally `jarAsmenys`                   | `pirkimoNumeris` links the award result to procurement; `tiekejoKodas` and `tiekejaiKodai` identify primary and additional winners               | No separate canonical award view: the winning relationship is inferred from published contracts; one procurement can have multiple contracts and winners |
| Contract                       | `v_sutartys`   | `vpmSutartys` and its type, category, party, CPV, and update tables                                          | Contract key is `sutartiesUnikalusId`; `v_sutartys.pirkimoNumeris = v_pirkimas.pirkimoId` links it back to procurement                           | A contract is the execution-stage result of an award and does not replace the procurement record; `pirkimoNumeris` may be absent                         |
