...
>
> **Loser (Bid) coverage is best-effort.** Loser participant data comes from `xlsxAtn1dalyviai` via
> `xlsxAtn1ataskaitos.pirkimoNumeris`. Only ~425 of 37,797 procurements have ATN1 data in the DB. When
> no ATN1 data exists for a procurement, only winner `Award` edges are shown — this is normal and
> expected. `xlsxAtn1dalyviai.kodas` maps to `jarAsmenys.jarKodas` for Lithuanian companies (`salis = 'LT'`).
...
| `Bidder`                                | Procurement → Org      | thin, **red**   | `xlsxAtn1ataskaitos JOIN xlsxAtn1dalyviai WHERE pirkimoNumeris = $pirkimoId AND salis='LT'` — procurement participants who did not win. Color `#ef4444`. Size 1. Best-effort: only ~425 procurements have ATN1 data. |
...
| `Bidder`                                               | `xlsxAtn1dalyviai`       | `null`                                                    | `null`                                                | No date columns in `xlsxAtn1dalyviai`; always open-ended.                                       |
