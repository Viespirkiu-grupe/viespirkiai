...
export async function loadAtn1ByFailasId(failasId: string | number): Promise<Atn1 | null> {
  const { rows } = await postgres.query(
    `SELECT * FROM "xlsxAtn1ataskaitos" WHERE "failasId" = $1 ORDER BY "sukurtaAt" DESC LIMIT 1`,
    [failasId],
  );
  ...
  const [pirkimoDalys, dalyviai, vertinimoKriterjai, atmestiPasiulymai, pasiulymuEile, proceduruPabaiga, sutartys] =
    await Promise.all([
      postgres.query(`SELECT * FROM "xlsxAtn1pirkimoDalys" WHERE "ataskaitaId" = $1 ORDER BY id`, [id]),
      postgres.query(`SELECT * FROM "xlsxAtn1dalyviai" WHERE "ataskaitaId" = $1 ORDER BY id`, [id]),
      postgres.query(`SELECT * FROM "xlsxAtn1vertinimoKriterjai" WHERE "ataskaitaId" = $1 ORDER BY id`, [id]),
      postgres.query(`SELECT * FROM "xlsxAtn1atmestiPasiulymai" WHERE "ataskaitaId" = $1 ORDER BY id`, [id]),
      postgres.query(`SELECT * FROM "xlsxAtn1pasiulymuEile" WHERE "ataskaitaId" = $1 ORDER BY id`, [id]),
      postgres.query(`SELECT * FROM "xlsxAtn1proceduruPabaiga" WHERE "ataskaitaId" = $1 ORDER BY id`, [id]),
      postgres.query(`SELECT * FROM "xlsxAtn1sutartys" WHERE "ataskaitaId" = $1 ORDER BY id`, [id]),
    ]);
 ...
 * `viesiejiPirkimai.pirkimoId` (matched on `xlsxAtn1ataskaitos.pirkimoNumeris`) and
...
export async function loadAtn1ForPirkimas(pirkimoNumeris: string): Promise<Atn1Santrauka | null> {
  const { rows } = await postgres.query(
    `SELECT id, "failasId" FROM "xlsxAtn1ataskaitos" WHERE "pirkimoNumeris" = $1 ORDER BY "sukurtaAt" DESC LIMIT 1`,
    [pirkimoNumeris],
  );
  const ataskaita = rows[0];
  if (!ataskaita) return null;
  ...
  const [dalyviaiRes, eileRes] = await Promise.all([
    postgres.query(`SELECT * FROM "xlsxAtn1dalyviai" WHERE "ataskaitaId" = $1 ORDER BY id`, [ataskaita.id]),
    postgres.query(`SELECT * FROM "xlsxAtn1pasiulymuEile" WHERE "ataskaitaId" = $1 ORDER BY "daliesNumeris", "eileNumeris"`, [ataskaita.id]),
  ]);
