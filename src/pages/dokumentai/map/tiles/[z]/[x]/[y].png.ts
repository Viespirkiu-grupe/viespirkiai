...
// Document heatmap tiles — same shape as /juridiniai/map/tiles, but backed by
// public."dokumentaiLocationTiles" (built + kept in sync by
// sql/dokumentaiLocationTiles.sql).
const TILE_SIZE = 256;
const OVERSAMPLE = 4;
...
export const GET: APIRoute = async ({ params }) => {
  ...
  const { rows } = await postgres.query(
    `SELECT "tileX", "tileY", "pointCount"
     FROM public."dokumentaiLocationTiles"
